import { StyleSheet, Platform, Dimensions } from "react-native";
import scale, { verticalScale } from "../../../components/src/Scale";
// Customizable Area Start
import { colors, lightTheme, redesignTheme } from "../../utilities/src/Colors";

type HomeTheme = typeof redesignTheme;

/** Shared home-feed spacing so search, featured, filters, and cards share one grid. */
const FEED_GUTTER = 16;
const FEED_SECTION_GAP = 16;
const FEED_CHIP_GAP = 8;
const FEED_CARD_GAP = 12;
const FEED_CARD_PAD = 12;
const DETAIL_HERO_HEIGHT = Math.round(Dimensions.get('window').height * 0.32);

export const createAllEventStyles = (theme: HomeTheme) => StyleSheet.create({
  container: {
    flex: 1,
    backgroundColor: theme.background
  },
  /** Top app bar on the events feed; dark redesign chrome. */
  feedHeaderBar: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
    backgroundColor: theme.background,
    paddingHorizontal: FEED_GUTTER,
    height: 56,
    borderBottomWidth: StyleSheet.hairlineWidth,
    borderBottomColor: theme.border,
  },
  feedHeaderBrandRow: {
    flexDirection: 'row',
    alignItems: 'center',
    flexShrink: 1,
    paddingRight: 8,
  },
  feedHeaderLogo: {
    width: 28,
    height: 28,
    borderRadius: 8,
    backgroundColor: theme.primary,
    alignItems: 'center',
    justifyContent: 'center',
    marginRight: 8,
  },
  feedHeaderTitle: {
    fontWeight: '900',
    fontSize: 18,
    letterSpacing: 0.6,
    color: theme.foreground,
    textTransform: 'uppercase',
  },
  feedHeaderLocationPill: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: theme.input,
    borderWidth: StyleSheet.hairlineWidth,
    borderColor: theme.border,
    borderRadius: 16,
    paddingHorizontal: 10,
    paddingVertical: 6,
    marginLeft: 8,
    maxWidth: 140,
  },
  feedHeaderLocationText: {
    color: theme.foreground,
    fontSize: 12,
    fontWeight: '600',
    marginLeft: 4,
    marginRight: 2,
    flexShrink: 1,
  },
  feedHeaderActions: {
    flexDirection: 'row',
    alignItems: 'center',
    marginLeft: 'auto',
  },
  feedHeaderIcon: {
    height: 22,
    width: 22,
    resizeMode: 'contain',
    tintColor: theme.foreground,
  },
  feedHeaderMenuIcon: {
    height: 18,
    width: 18,
    resizeMode: 'contain',
    marginLeft: 10,
    tintColor: theme.foreground,
  },
  feedHeaderBackIcon: {
    width: 12,
    height: 12,
    resizeMode: 'contain',
    tintColor: theme.foreground,
  },
  feedHeaderAvatarWrap: {
    marginLeft: 10,
    width: 32,
    height: 32,
    borderRadius: 16,
    borderWidth: 1.5,
    borderColor: theme.primary,
    overflow: 'hidden',
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: theme.input,
  },
  feedHeaderAvatar: {
    width: 32,
    height: 32,
    borderRadius: 16,
  },
  /** Groups category filters and sort/state controls under the redesign search. */
  feedToolbarSurface: {
    marginHorizontal: 0,
    backgroundColor: theme.background,
    borderWidth: 0,
    overflow: 'visible',
    paddingBottom: 0,
  },
  /** Aligns with feedToolbarSurface so list body shares the same side rails. */
  feedListContent: {
    backgroundColor: theme.background,
    flexGrow: 1,
  },
  /** Centered spinner in the feed body while the shows API is in flight (empty list). */
  feedListLoadingEmpty: {
    flexGrow: 1,
    minHeight: 320,
    justifyContent: 'center',
    alignItems: 'center',
    backgroundColor: theme.background,
    paddingVertical: 40,
  },
  input: {
    flex: 1,
    height: 44,
    fontSize: 14,
    marginLeft: 8,
    color: theme.foreground,
    paddingVertical: 0,
  },
  inputContainer: {
    flexDirection: 'row',
    alignItems: 'center',
    borderRadius: 16,
    paddingVertical: 4,
    paddingLeft: 12,
    paddingRight: 6,
    marginHorizontal: FEED_GUTTER,
    marginTop: 12,
    marginBottom: FEED_SECTION_GAP,
    backgroundColor: theme.input,
    borderWidth: StyleSheet.hairlineWidth,
    borderColor: theme.border,
  },
  nearMeButton: {
    backgroundColor: theme.primarySoft,
    borderRadius: 10,
    paddingHorizontal: 10,
    paddingVertical: 8,
    marginLeft: 6,
  },
  nearMeButtonText: {
    color: theme.primary,
    fontSize: 12,
    fontWeight: '600',
  },
  featuredCard: {
    marginHorizontal: FEED_GUTTER,
    marginBottom: FEED_SECTION_GAP,
    borderRadius: 18,
    overflow: 'hidden',
    backgroundColor: theme.card,
    borderWidth: StyleSheet.hairlineWidth,
    borderColor: theme.border,
    height: 190,
  },
  featuredImage: {
    width: '100%',
    height: '100%',
  },
  featuredImagePlaceholder: {
    width: '100%',
    height: '100%',
    backgroundColor: theme.input,
    alignItems: 'center',
    justifyContent: 'center',
  },
  featuredOverlay: {
    ...StyleSheet.absoluteFillObject,
    backgroundColor: 'rgba(8, 8, 15, 0.12)',
  },
  featuredBottomFade: {
    position: 'absolute',
    left: 0,
    right: 0,
    bottom: 0,
    height: 72,
    backgroundColor: 'rgba(8, 8, 15, 0.38)',
  },
  featuredBadge: {
    position: 'absolute',
    top: FEED_CARD_PAD,
    left: FEED_CARD_PAD,
    backgroundColor: theme.primary,
    borderRadius: 999,
    paddingHorizontal: 10,
    paddingVertical: 4,
  },
  featuredBadgeText: {
    color: '#FFFFFF',
    fontSize: 9,
    fontWeight: '700',
    letterSpacing: 0.6,
  },
  featuredContentRow: {
    position: 'absolute',
    left: FEED_CARD_PAD,
    right: FEED_CARD_PAD,
    bottom: FEED_CARD_PAD,
    flexDirection: 'row',
    alignItems: 'flex-end',
    justifyContent: 'space-between',
  },
  featuredTextBlock: {
    flex: 1,
    paddingRight: 12,
  },
  featuredTitle: {
    color: '#FFFFFF',
    fontSize: 22,
    fontWeight: '900',
    letterSpacing: 0.4,
    textTransform: 'uppercase',
  },
  featuredMetaRow: {
    flexDirection: 'row',
    alignItems: 'center',
    marginTop: 4,
    flexWrap: 'wrap',
  },
  featuredMetaText: {
    color: 'rgba(240, 238, 255, 0.85)',
    fontSize: 12,
    fontWeight: '500',
    marginLeft: 4,
  },
  featuredTimeText: {
    color: theme.featuredTime,
    fontSize: 12,
    fontWeight: '700',
  },
  featuredTicketButton: {
    width: 44,
    height: 44,
    borderRadius: 22,
    backgroundColor: theme.primary,
    alignItems: 'center',
    justifyContent: 'center',
    ...Platform.select({
      ios: {
        shadowColor: theme.primary,
        shadowOffset: { width: 0, height: 0 },
        shadowOpacity: 0.7,
        shadowRadius: 10,
      },
      android: {
        elevation: 6,
      },
    }),
  },
  button: {
    borderRadius: 10,
    alignItems: 'center',
  },
  verticalSeperator: {
    height: 20,
    width: 1,
    backgroundColor: '#909090',
    marginLeft: 30,
  },
  centeredView: {
    flex: 1,
    justifyContent: 'flex-end',
  },
  pickerSheetOverlay: {
    flex: 1,
    justifyContent: 'flex-end',
    backgroundColor: 'rgba(8, 8, 15, 0.72)',
  },
  pickerSheetContainer: {
    backgroundColor: theme.card,
    borderTopLeftRadius: 24,
    borderTopRightRadius: 24,
    borderWidth: 1,
    borderColor: theme.border,
    paddingTop: 10,
    paddingBottom: 28,
    paddingHorizontal: 16,
    maxHeight: '62%',
  },
  pickerSheetHandle: {
    alignSelf: 'center',
    width: 40,
    height: 4,
    borderRadius: 2,
    backgroundColor: theme.muted,
    opacity: 0.5,
    marginBottom: 14,
  },
  pickerSheetTitle: {
    fontFamily: 'OpenSans',
    fontWeight: '700',
    fontSize: 18,
    color: theme.primary,
    marginBottom: 12,
    textAlign: 'center',
  },
  pickerSheetList: {
    maxHeight: 340,
  },
  pickerSheetOption: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    paddingVertical: 14,
    paddingHorizontal: 16,
    borderRadius: 14,
    marginBottom: 8,
    backgroundColor: theme.input,
    borderWidth: 1,
    borderColor: theme.border,
  },
  pickerSheetOptionSelected: {
    backgroundColor: theme.primarySoft,
    borderColor: theme.primary,
  },
  pickerSheetOptionText: {
    fontFamily: 'OpenSans',
    fontSize: 16,
    fontWeight: '600',
    color: theme.foreground,
    textAlign: 'center',
  },
  pickerSheetOptionTextSelected: {
    color: theme.primary,
  },
  hiddenPicker: {
    height: 0,
    width: 0,
    opacity: 0,
    position: 'absolute',
  },
  modalView: {
    height: '40%',
    justifyContent: 'space-between',
    backgroundColor: theme.card,
    borderTopEndRadius: 20,
    padding: 35,
    shadowColor: '#000',
    shadowOffset: {
      width: 0,
      height: 2,
    },
    shadowOpacity: 0.25,
    shadowRadius: 4,
    elevation: 5,
  },
  loginSheet: {
    justifyContent: 'flex-start',
    backgroundColor: theme.card,
    borderTopLeftRadius: 24,
    borderTopRightRadius: 24,
    borderWidth: StyleSheet.hairlineWidth,
    borderColor: theme.border,
    paddingTop: 20,
    paddingHorizontal: 24,
    paddingBottom: 32,
    shadowColor: '#000',
    shadowOffset: {
      width: 0,
      height: -2,
    },
    shadowOpacity: 0.25,
    shadowRadius: 8,
    elevation: 8,
  },
  textStyle: {
    color: 'white',
    fontWeight: 'bold',
    textAlign: 'center',
  },
  detailContainer: {
    backgroundColor: "#1a1a1a",
    margin: scale(25),
    borderRadius: 10,
    padding: scale(20),
  },
  detailProfileContainer: {
    flexDirection: "row",
    justifyContent: "space-between",
  },
  detailProfileContent: {
    flexDirection: "row",
    justifyContent: "space-around",
  },
  detailProfileImageContent: {
    width: 60,
    height: 60,
    borderRadius: 30,
    backgroundColor: "#878787",
  },
  detailProfileImage: {
    width: "100%",
    height: "100%",
    borderRadius: 30,
  },
  detailProfileTitleContent: {
    marginLeft: scale(10),
  },
  detailProfileTitle: {
    fontSize: 14,
    color: "#fff",
  },
  detailProfileEventTypeContent: {
    justifyContent: "center",
    flexDirection: "row",
    alignItems: "center",
    alignSelf: "stretch",
    backgroundColor: "#70ff73",
    paddingHorizontal: 5,
    paddingVertical: 3,
    borderRadius: 6,
    marginTop: scale(10),
  },
  detailProfileEventTypeImage: {
    width: 15,
    height: 15,
    marginRight: scale(6),
  },
  detailProfileEventTypeText: {
    fontSize: 12,
    color: "#000",
    fontWeight: "500",
    marginTop: Platform.OS === "ios" ? scale(4) : 0,
  },
  detailProfileUpcomingTitle: {
    fontSize: 14,
    color: "#fff",
    marginLeft: 15,
  },
  detailProfileDateTimeContent: {
    padding: 20,
    justifyContent: "center",
    alignItems: "center",
  },
  detailProfileDateTimeHeaderContent: {
    flexDirection: "row",
    justifyContent: "space-between",
    width: "60%",
  },
  detailProfileDateTimeHeaderTime: {
    fontSize: 14,
    color: "#fff",
    textAlign: "right",
  },
  detailProfileDateTimeHeaderDate: {
    fontSize: 14,
    color: "#fff",
  },
  detailProfileDateTimeValueContent: {
    flexDirection: "row",
    justifyContent: "space-between",
    width: "75%",
    marginTop: scale(10),
  },
  detailProfileDateTimeValue: {
    fontSize: 16,
    color: "#fff",
    fontWeight: "bold",
  },
  detailProfileDetailsContainer: {
    padding: 15,
  },
  detailProfileDetailsContent: {
    marginTop: scale(8),
    flexDirection: "row",
    alignItems: "center",
  },
  detailProfileDetailsTitle: {
    fontSize: 16,
    color: "#fff",
    fontWeight: "bold",
  },
  detailProfileDetailsDesc: {
    fontSize: 16,
    color: "#fff",
    opacity: 0.6,
  },
  editImage: {
    width: scale(18),
    height: scale(18),
    marginTop: verticalScale(2),
  },
  dateBanner: {
    position: 'absolute',
    top: 20,
    right: 20,
    paddingVertical: 7,
    paddingHorizontal: 15,
    borderRadius: 10,
    backgroundColor: '#4949EE',
    flexWrap: 'wrap',
    display: 'flex',
    flexDirection: 'column',
    alignItems: 'center',
    justifyContent: 'center',
  },
  eventDetails: {
    flexDirection: 'row',
    flex: 0.4,
    alignItems: 'center'
  },
  detailIcons: {
    width: 15,
    height: 15,
    resizeMode: 'contain'
  },
  boldText: {
    fontWeight: '700',
    fontSize: 14,
    color: '#334155',
  },
  loadingContainer: {
    ...StyleSheet.absoluteFillObject,
    backgroundColor: '#ffffffdd',
    alignItems: "center",
    justifyContent: "center",
  },
  textInput: {
    alignSelf: 'flex-end',
    paddingHorizontal: 10,
    color: colors(false).text,
    height: 50,
  },
  selector: {
    height: 50,
    justifyContent: "center",
    paddingHorizontal: 0,
  },
  downArrow: {
    width: 15,
    height:15,
    marginLeft: 10,
    marginBottom:15,
    backgroundColor: "#F1F5F9",
    transform: [{ rotate: "-90deg" }],
    resizeMode: "contain",
  },
  sortByRow: {
    flexDirection: 'row',
    alignItems: 'center',
    paddingHorizontal: 10,
    marginTop: 10,
    width: '100%',
  },
  sortByCell: {
    flex: 1,
    minWidth: 100,
  },
  sortByCellFirst: {
    flex: 1.4,
    minWidth: 120,
    marginRight: 8,
  },
  sortByCellLast: {
    flex: 1,
    minWidth: 90,
  },
  sortByTouchable: {
    flexDirection: 'row',
    alignItems: 'center',
    height: 40,
    paddingRight: 10,
    paddingLeft: 10,
    justifyContent: 'flex-start',
    minWidth: 0,
    alignSelf: 'flex-start',
  },
  sortByLabel: {
    flexShrink: 1,
    minWidth: 0,
    fontSize: 14,
  },
  sortByLabelText: {
    marginRight: 5,
  },
  sortByContainerAndroid: {
    flexDirection: 'row',
    alignItems: 'center',
    paddingHorizontal: 8,
    height: 40,
    minWidth: 0,
    alignSelf: 'flex-start',
    position: 'relative',
  },
  sortByAndroidLabelRow: {
    flexDirection: 'row',
    alignItems: 'center',
    paddingRight: 10,
  },
  sortByLabelAndroid: {
    fontSize: 14,
    minWidth: 0,
  },
  downArrowAndroid: {
    width: 8,
    marginLeft: 4,
    backgroundColor: '#F1F5F9',
    transform: [{ rotate: '-90deg' }],
    resizeMode: 'contain',
  },
  sortByPickerOverlay: {
    ...StyleSheet.absoluteFillObject,
    justifyContent: 'center',
  },
  sortByPickerInvisible: {
    height: 40,
    width: '100%',
    opacity: 0,
  },
  sortByPickerWrap: {
    flex: 1,
    minWidth: 80,
    // backgroundColor:"red",
    height:100
  },
  sortByPicker: {
    flex: 1,
    height: 40,
    marginLeft: -4,
  },
  txt: {
    fontFamily: "OpenSans",
    alignSelf: "flex-start",
    color: theme.foreground,
    fontWeight:"600"
  },
  welcomeToPopupText: {
    textAlign: 'center',
    fontSize: 16,
    lineHeight: 22,
    marginTop: 28,
    color: theme.muted,
  },
  localShowsPopupText: {
    fontWeight: '800',
    fontSize: 32,
    lineHeight: 38,
    textAlign: 'center',
    color: theme.primary,
    marginTop: 4,
    marginBottom: 12,
  },
  loginFirstPopupText: {
    textAlign: 'center',
    lineHeight: 22,
    marginBottom: 20,
    fontWeight: '400',
    fontSize: 15,
    color: theme.muted,
    paddingHorizontal: 12,
  },
  createAccountPopupText: {
    color: theme.primary,
    fontWeight: '700',
    lineHeight: 24,
    marginBottom: 20,
    fontSize: 16,
    textAlign: 'center',
  },
  loginBtnPopupText: {
    backgroundColor: theme.primary,
    borderRadius: 10,
    height: 56,
    justifyContent: 'center',
    alignItems: 'center',
  },
  loginTxtPopupText: {
    color: '#FFFFFF',
    fontWeight: '700',
    textAlign: 'center',
    fontSize: 16,
  },
  headerContainer: {
    flexDirection: "row",
    justifyContent: "space-between",
    alignItems: "center",
    backgroundColor: "white",
    height: 50,
  },
  headerIconBtn : {
    flexDirection: "row",
    alignItems: 'center',
    width: 40,
    height: 40
},
  backButton: {
    width: 20,
  },
  arrowIcon: {
    width: 12,
    left: 0,
    resizeMode: "contain",
  },
  headerText: {
    fontWeight: "700",
    fontSize: 24,
    color: theme.foreground,
    textAlign: "center",
    textAlignVertical:'center',
    maxWidth:'70%'
  },
  hamburgerButton: {
    
  },
  hamburgerIcon: {
    height: 20,
    width: 20,
    resizeMode: "contain",
  
  },
  image: {
    width: '100%',
    height: 270,
    borderRadius: 10,
    marginBottom: 10,
  },
  threeDotsButton: {
    position: 'absolute',
    right: 10,
    top: 10,
    padding: 10,
  },
  threeDotsIcon: {
    width: 30,
    height: 30,
    resizeMode: 'contain',
  },
  divider: {
    height: "50%",
    width: 1,
    backgroundColor: "#CBD5E1",
    alignSelf: 'center',
  },
  gallery: {
    height: 40,
    width: 40,
    resizeMode: 'contain',
    tintColor: '#00000050',
  },
  menuContainer: {
    position: 'absolute',
    right: 20,
    top: 60,
    backgroundColor: theme.card,
    borderRadius: 10,
    padding: 10,
    paddingLeft: 30,
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.9,
    shadowRadius: 3,
    elevation: 3,
  },
  menuButton: {
    paddingHorizontal: 15,
    paddingVertical: 10,
  },
  menuButtonText: {
    fontSize: 16,
    fontWeight: '400',
    color: theme.foreground,
    textAlignVertical: 'center',
  },
  commentsParentView: {
    backgroundColor: '#33415580',
  },
  commentsView: {
    height: '75%',
    padding: 20,
    justifyContent: 'flex-start',
  },
  commentsHeader: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center'
  },
  commentsHeadingText: {
    fontSize: 24,
    fontWeight: 'bold',
    color: theme.foreground,
  },
  horizontalRuler: {
    width: '100%',
    height: 1,
    backgroundColor: theme.divider,
    marginVertical: 15,
  },
  commentsListContainer: {
    flex: 1,
  },
  emojiSelectionBar: {
    height: 40,
    flexDirection: 'row',
    justifyContent: 'space-between',
  },
  emojiSelectionBarIcon: {
    fontSize: 20,
  },
  commentInputArea: {
    height: 50,
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center'
  },
  userAvatar: {
    width: 40,
    height: 40,
    borderRadius: 75,
  },
  userAvatarContainer: {
    backgroundColor: theme.input,
    width: 42,
    height: 42,
    borderRadius: 80,
    alignSelf: "center",
    alignItems: "center",
    justifyContent: "center",
    borderWidth: 1,
    borderColor: theme.border,
    marginRight: 10,
  },
  commentTextInput: {
    flex: 1,
    borderWidth: 1,
    borderColor: theme.border,
    backgroundColor: theme.input,
    color: theme.foreground,
    borderRadius: 10,
    paddingHorizontal: 10,
    paddingRight: 50,
    paddingTop: 12,
    height: '100%',
    fontSize: 16,
    textAlignVertical: 'center'
  },
  emojiButton: {
    position: 'absolute',
    right: 10,
    height: 30,
    width: 30,
  },
  emojiIcon: {
    width: 30,
    height: 30,
    resizeMode: 'contain',
  },
  noCommentsContainer: {
    flex: 1,
    justifyContent: 'center',
    alignItems: 'center',
    paddingVertical: 16,
  },
  startConversationText: {
    marginTop: 5,
    color: theme.muted,
  },
  commentItem: {
    flexDirection: "row",
    justifyContent: "space-between",
    marginVertical: 10,
  },
  commentContent: {
    flex: 1
  },
  commenterName: {
    fontSize: 16,
    fontWeight: "bold",
    color: theme.foreground,
  },
  commentText: {
    fontSize: 14,
    color: theme.foreground,
  },
  likeContainer: {
    alignSelf: "flex-start",
    alignItems: "center",
    justifyContent: "center",
  },
  likesCountText: {
    color: theme.muted,
    textAlign: "center",
  },
  replyButton: {
    alignSelf: "flex-start",
  },
  replyButtonText: {
    fontWeight: "bold",
    fontSize: 14,
    color: theme.muted,
  },
  showReplyButton: {
    flexDirection: "row",
    alignSelf: "flex-start",
    alignItems: "center",
  },
  horizontalBar: {
    height: 1,
    width: 20,
    marginRight: 10,
    backgroundColor: theme.muted,
  },
  showReplyButtonText: {
    fontSize: 14,
    color: theme.muted
  },
  replyText: {
    marginTop: 10,
  },
  rowBack: {
    alignItems: 'center',
    flex: 1,
    flexDirection: 'row',
    justifyContent: 'space-between',
    marginVertical: 10,
  },
  rowFront: {
    alignItems: 'center',
    backgroundColor: theme.card,
    width: '100%',
    flexDirection: "row",
    justifyContent: "space-between",
    marginVertical: 10,
    paddingRight: 5,
  },
  backRightBtn: {
    alignItems: 'center',
    bottom: 0,
    justifyContent: 'center',
    position: 'absolute',
    top: 0,
    width: 60,
  },
  backRightBtnLeft: {
    backgroundColor: '#33415580',
    right: 60,
  },
  backRightBtnRight: {
    backgroundColor: '#DC2626',
    right: 0,
  },
  redDot: {
    height: 8,
    width: 8,
    borderRadius: 5,
    backgroundColor: "red",
    position: "absolute",
    right: -4,
    top: 2,
  },
  notificationWrapper: {
    position: 'relative',
    justifyContent: 'center',
    alignItems: 'center',
  },
  notificationBadge: {
    position: 'absolute',
    top: -6,
    right: -8,
    minWidth: 18,
    height: 18,
    borderRadius: 9,
    backgroundColor: '#F04438',
    justifyContent: 'center',
    alignItems: 'center',
    paddingHorizontal: 4,
  },
  notificationBadgeText: {
    color: '#FFFFFF',
    fontSize: 10,
    fontWeight: '700',
  },
  disablePopupIconContainer: {
    position: "absolute",
    right: 20,
    top: 20,
  },
  textDeleteHeading: {
    fontWeight: "700",
    fontSize: 26,
    lineHeight: 28,
    marginBottom: 10,
    marginTop: 25,
    color: theme.foreground,
  },
  textDelete: {
    fontSize: 18,
    color: theme.muted,
  },
  deleteButtonContainer: {
    backgroundColor: theme.primary,
    width: "100%",
    padding: 15,
    borderRadius: 10,
    marginTop: 15,
  },
  textDeleteButton: {
    color: colors(false).white,
    fontWeight: "700",
    fontSize: 18,
    alignSelf: "center",
  },
  keepButtonContainer: {
    backgroundColor: "transparent",
    borderWidth: 1,
    borderColor: theme.divider,
  },
  textKeepButton: {
    color: theme.foreground,
  },
  filterButton: {
    borderRadius: 999,
    paddingHorizontal: 14,
    paddingVertical: 9,
    justifyContent: 'center',
    alignItems: 'center',
    flexDirection: 'row',
    backgroundColor: theme.input,
    borderWidth: 1,
    borderColor: theme.border,
    marginRight: 8,
  },
  filterButtonActive: {
    backgroundColor: theme.primary,
    borderColor: theme.primary,
  },
  filterButtonText: {
    fontSize: 12,
    fontWeight: '600',
    color: theme.muted,
  },
  filterButtonTextActive: {
    color: '#FFFFFF',
  },
  filterPillsRow: {
    paddingLeft: FEED_GUTTER,
    paddingRight: 0,
    marginBottom: FEED_CHIP_GAP,
  },
  filterPillsRowLast: {
    paddingLeft: FEED_GUTTER,
    paddingRight: 0,
    marginBottom: 0,
  },
  filterPillsContent: {
    paddingRight: FEED_GUTTER,
  },
  guestSignupBanner: {
    marginHorizontal: FEED_GUTTER,
    marginTop: 4,
    marginBottom: FEED_SECTION_GAP,
    minHeight: 52,
    borderRadius: 999,
    backgroundColor: 'rgba(58, 22, 48, 0.92)',
    borderWidth: StyleSheet.hairlineWidth,
    borderColor: 'rgba(255, 45, 107, 0.22)',
    flexDirection: 'row',
    alignItems: 'center',
    paddingLeft: 8,
    paddingRight: 10,
    paddingVertical: 6,
  },
  guestSignupStarWrap: {
    width: 36,
    height: 36,
    borderRadius: 18,
    backgroundColor: 'rgba(28, 12, 24, 0.9)',
    alignItems: 'center',
    justifyContent: 'center',
    marginRight: 8,
  },
  guestSignupBannerText: {
    flex: 1,
    color: '#FFFFFF',
    fontSize: 13,
    fontWeight: '700',
    paddingRight: 8,
  },
  guestSignupJoinButton: {
    backgroundColor: theme.primary,
    borderRadius: 999,
    paddingHorizontal: 16,
    height: 32,
    alignItems: 'center',
    justifyContent: 'center',
    marginRight: 8,
  },
  guestSignupJoinText: {
    color: '#FFFFFF',
    fontSize: 13,
    fontWeight: '700',
  },
  guestSignupCloseButton: {
    width: 22,
    height: 22,
    alignItems: 'center',
    justifyContent: 'center',
  },
  sortPill: {
    borderRadius: 999,
    paddingHorizontal: 14,
    paddingVertical: 8,
    backgroundColor: theme.input,
    borderWidth: 1,
    borderColor: theme.border,
    marginRight: 8,
  },
  sortPillActive: {
    backgroundColor: theme.primarySoft,
    borderColor: theme.primary,
  },
  sortPillText: {
    fontSize: 12,
    fontWeight: '600',
    color: theme.muted,
  },
  sortPillTextActive: {
    color: theme.primary,
  },
  reportModalContent: {
    height: 'auto',
    maxHeight: '85%',
    width: '90%',
    alignSelf: 'center',
    padding: 24,
    borderTopLeftRadius: 20,
    borderTopRightRadius: 20,
    borderBottomLeftRadius: 20,
    borderBottomRightRadius: 20,
  },
  reportHeader: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  },
  reportTitle: {
    fontWeight: '700',
    fontSize: 20,
    color: theme.foreground,
  },
  reportSubtitle: {
    marginTop: 16,
    fontSize: 14,
    color: theme.muted,
    fontWeight: '500',
  },
  reportReasonsContainer: {
    marginTop: 12,
  },
  reportReasonButton: {
    flexDirection: 'row',
    alignItems: 'center',
    paddingVertical: 12,
    paddingHorizontal: 12,
    borderRadius: 10,
    borderWidth: 1,
    borderColor: theme.border,
    marginBottom: 10,
    backgroundColor: theme.input,
  },
  reportReasonButtonSelected: {
    borderColor: theme.primary,
    backgroundColor: theme.primarySoft,
  },
  reportRadioOuter: {
    width: 20,
    height: 20,
    borderRadius: 10,
    borderWidth: 1.5,
    borderColor: theme.muted,
    alignItems: 'center',
    justifyContent: 'center',
    marginRight: 12,
  },
  reportRadioOuterSelected: {
    borderColor: theme.primary,
  },
  reportRadioInner: {
    width: 10,
    height: 10,
    borderRadius: 5,
    backgroundColor: theme.primary,
  },
  reportReasonText: {
    fontSize: 14,
    color: theme.foreground,
    flex: 1,
  },
  reportReasonTextSelected: {
    color: theme.foreground,
    fontWeight: '600',
  },
  reportErrorText: {
    color: theme.primary,
    fontSize: 12,
    marginTop: 4,
  },
  reportCommentInput: {
    marginTop: 12,
    borderWidth: 1,
    borderColor: theme.border,
    backgroundColor: theme.input,
    borderRadius: 10,
    padding: 12,
    minHeight: 100,
    textAlignVertical: 'top',
    color: theme.foreground,
  },
  reportActions: {
    flexDirection: 'row',
    justifyContent: 'flex-end',
    marginTop: 20,
  },
  reportButton: {
    paddingVertical: 12,
    paddingHorizontal: 20,
    borderRadius: 8,
  },
  reportCancelButton: {
    backgroundColor: theme.input,
    marginRight: 12,
  },
  reportCancelText: {
    color: theme.foreground,
    fontWeight: '600',
  },
  reportSubmitButton: {
    backgroundColor: theme.primary,
  },
  reportSubmitText: {
    color: '#FFFFFF',
    fontWeight: '700',
  },
  closeBtn:{
    padding: 8,
    position: 'absolute',
    top: 12,
    right: 12,
    width: 32,
    height: 32,
    justifyContent: 'center',
    alignItems: 'center',
    zIndex: 2,
  },
  showsCountRow: {
    flexDirection: 'row',
    alignItems: 'center',
    marginHorizontal: FEED_GUTTER,
    marginTop: FEED_SECTION_GAP,
    marginBottom: FEED_SECTION_GAP,
  },
  showsCountLabelWrap: {
    flexDirection: 'row',
    alignItems: 'center',
    marginRight: 10,
  },
  showsCountDot: {
    width: 6,
    height: 6,
    borderRadius: 3,
    backgroundColor: theme.primary,
    marginRight: 6,
  },
  showsCountText: {
    color: theme.primary,
    fontSize: 12,
    fontWeight: '800',
    letterSpacing: 0.8,
  },
  showsCountLine: {
    flex: 1,
    height: StyleSheet.hairlineWidth,
    backgroundColor: theme.divider,
    marginRight: 10,
  },
  showsCountArea: {
    color: theme.muted,
    fontSize: 12,
    fontWeight: '600',
  },
  compactShowCard: {
    flexDirection: 'column',
    marginHorizontal: FEED_GUTTER,
    marginBottom: FEED_CARD_GAP,
    padding: FEED_CARD_PAD,
    borderRadius: 16,
    backgroundColor: theme.card,
    borderWidth: StyleSheet.hairlineWidth,
    borderColor: theme.border,
  },
  compactShowMainRow: {
    flexDirection: 'row',
    alignItems: 'flex-start',
  },
  compactShowThumbWrap: {
    width: 108,
    height: 108,
    borderRadius: 14,
    overflow: 'hidden',
    backgroundColor: theme.input,
  },
  compactShowThumb: {
    width: 108,
    height: 108,
  },
  compactShowThumbPlaceholder: {
    width: 108,
    height: 108,
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: theme.input,
  },
  compactShowBody: {
    flex: 1,
    marginLeft: FEED_CARD_PAD,
    minWidth: 0,
    minHeight: 108,
  },
  compactShowTitleRow: {
    flexDirection: 'row',
    alignItems: 'flex-start',
    justifyContent: 'space-between',
  },
  compactShowTitle: {
    flex: 1,
    color: theme.foreground,
    fontSize: 16,
    fontWeight: '800',
    letterSpacing: 0.2,
    textTransform: 'uppercase',
    paddingRight: 8,
  },
  compactShowGenre: {
    color: theme.muted,
    fontSize: 12,
    fontWeight: '500',
    marginTop: 2,
  },
  compactShowLineup: {
    color: theme.muted,
    fontSize: 11,
    fontWeight: '500',
    marginTop: 2,
  },
  compactShowMetaRow: {
    flexDirection: 'row',
    alignItems: 'center',
    flexWrap: 'wrap',
    marginTop: 8,
  },
  compactShowVenue: {
    color: theme.foreground,
    fontSize: 12,
    fontWeight: '600',
    marginLeft: 4,
    flexShrink: 1,
  },
  compactShowTime: {
    color: theme.accent,
    fontSize: 12,
    fontWeight: '700',
    marginLeft: 8,
  },
  compactShowActions: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    marginTop: 8,
  },
  compactShowActionIcons: {
    flexDirection: 'row',
    alignItems: 'center',
  },
  compactShowActionIcon: {
    height: 18,
    width: 18,
    resizeMode: 'contain',
    tintColor: theme.muted,
    marginRight: 10,
  },
  compactShowLikeIcon: {
    height: 18,
    width: 20,
    resizeMode: 'contain',
    tintColor: theme.primary,
  },
  compactShowCalendarText: {
    color: theme.primary,
    fontWeight: '500',
    fontSize: 12,
  },
  compactShowSocialText: {
    color: theme.muted,
    fontSize: 12,
    fontWeight: '600',
    marginTop: 6,
  },
  compactShowSocialLink: {
    color: theme.primary,
    fontWeight: '400',
  },
  compactSeeMoreText: {
    fontWeight: '400',
    fontSize: 14,
    textDecorationLine: 'underline',
    paddingHorizontal: FEED_GUTTER,
    paddingTop: 4,
    paddingBottom: FEED_CARD_GAP,
    color: theme.primary,
    textAlign: 'right',
  },
  compactShowMoreFooterButton: {
    alignSelf: 'flex-end',
  },
  compactShowMoreFooter: {
    fontWeight: '600',
    fontSize: 14,
    textDecorationLine: 'underline',
    paddingRight: FEED_GUTTER,
    paddingLeft: 12,
    paddingTop: 12,
    paddingBottom: 12,
    color: theme.primary,
    textAlign: 'right',
  },
  discoveryFooter: {
    paddingBottom: 28,
  },
  discoverySection: {
    marginTop: 20,
    marginBottom: 8,
  },
  discoverySectionHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    paddingHorizontal: FEED_GUTTER,
    marginBottom: 14,
  },
  discoverySectionTitle: {
    color: theme.foreground,
    fontSize: 16,
    fontWeight: '800',
    letterSpacing: 0.8,
    flexShrink: 1,
    paddingRight: 12,
  },
  discoverySectionLink: {
    flexDirection: 'row',
    alignItems: 'center',
    flexShrink: 0,
  },
  discoverySectionLinkText: {
    color: theme.primary,
    fontSize: 13,
    fontWeight: '600',
    marginRight: 2,
  },
  discoveryRowContent: {
    paddingHorizontal: FEED_GUTTER,
  },
  artistWatchItem: {
    width: 88,
    alignItems: 'center',
    marginRight: 14,
  },
  artistWatchAvatarWrap: {
    width: 72,
    height: 72,
    marginBottom: 8,
  },
  artistWatchAvatar: {
    width: 72,
    height: 72,
    borderRadius: 36,
    backgroundColor: theme.input,
  },
  artistWatchAvatarPlaceholder: {
    width: 72,
    height: 72,
    borderRadius: 36,
    backgroundColor: theme.primarySoft,
    borderWidth: 1,
    borderColor: theme.primary,
    alignItems: 'center',
    justifyContent: 'center',
  },
  artistWatchBadge: {
    position: 'absolute',
    right: -1,
    bottom: -1,
    width: 22,
    height: 22,
    borderRadius: 11,
    backgroundColor: theme.primary,
    alignItems: 'center',
    justifyContent: 'center',
    borderWidth: 2,
    borderColor: theme.background,
  },
  artistWatchName: {
    color: theme.foreground,
    fontSize: 12,
    fontWeight: '700',
    textAlign: 'center',
    width: '100%',
  },
  artistWatchMeta: {
    color: theme.muted,
    fontSize: 11,
    fontWeight: '500',
    textAlign: 'center',
    marginTop: 2,
  },
  artistAllHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    paddingHorizontal: FEED_GUTTER,
    height: 56,
    borderBottomWidth: StyleSheet.hairlineWidth,
    borderBottomColor: theme.border,
    backgroundColor: theme.background,
  },
  artistAllBackButton: {
    width: 40,
    height: 40,
    alignItems: 'center',
    justifyContent: 'center',
  },
  artistAllHeaderTitle: {
    color: theme.foreground,
    fontSize: 16,
    fontWeight: '800',
    letterSpacing: 0.8,
    textAlign: 'center',
    flex: 1,
  },
  artistAllListContent: {
    paddingHorizontal: FEED_GUTTER,
    paddingTop: 8,
    paddingBottom: 28,
  },
  artistAllRow: {
    flexDirection: 'row',
    alignItems: 'center',
    paddingVertical: 12,
    borderBottomWidth: StyleSheet.hairlineWidth,
    borderBottomColor: theme.border,
  },
  artistAllAvatarWrap: {
    width: 56,
    height: 56,
    marginRight: 12,
    borderRadius: 28,
    overflow: 'hidden',
  },
  artistAllAvatar: {
    width: 56,
    height: 56,
    borderRadius: 28,
    backgroundColor: theme.input,
  },
  artistAllAvatarPlaceholder: {
    width: 56,
    height: 56,
    borderRadius: 28,
    backgroundColor: theme.input,
    alignItems: 'center',
    justifyContent: 'center',
  },
  artistAllTextWrap: {
    flex: 1,
    paddingRight: 8,
  },
  artistAllName: {
    color: theme.foreground,
    fontSize: 16,
    fontWeight: '700',
  },
  artistAllMeta: {
    color: theme.muted,
    fontSize: 13,
    fontWeight: '500',
    marginTop: 2,
  },
  artistAllEmpty: {
    color: theme.muted,
    fontSize: 14,
    textAlign: 'center',
    marginTop: 48,
  },
  hotVenueCard: {
    width: 210,
    height: 136,
    borderRadius: 16,
    overflow: 'hidden',
    marginRight: 12,
    backgroundColor: theme.card,
  },
  hotVenueImage: {
    width: '100%',
    height: '100%',
  },
  hotVenueScrim: {
    ...StyleSheet.absoluteFillObject,
    backgroundColor: 'rgba(8, 8, 15, 0.38)',
  },
  hotVenueTextWrap: {
    position: 'absolute',
    left: 12,
    right: 12,
    bottom: 12,
  },
  hotVenueName: {
    color: '#FFFFFF',
    fontSize: 15,
    fontWeight: '800',
    letterSpacing: 0.4,
    textTransform: 'uppercase',
  },
  hotVenueMeta: {
    color: 'rgba(255, 255, 255, 0.88)',
    fontSize: 12,
    fontWeight: '600',
    marginTop: 3,
  },
  hotVenueCount: {
    color: theme.accent,
    fontWeight: '800',
  },
  compactShowFooter: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    marginTop: FEED_CARD_PAD,
    paddingTop: FEED_CARD_PAD,
    borderTopWidth: StyleSheet.hairlineWidth,
    borderTopColor: theme.divider,
  },
  compactShowCategoryPill: {
    borderRadius: 999,
    paddingHorizontal: 14,
    paddingVertical: 6,
    borderWidth: StyleSheet.hairlineWidth,
    borderColor: theme.primary,
    backgroundColor: theme.primarySoft,
  },
  compactShowCategoryText: {
    color: theme.primary,
    fontSize: 13,
    fontWeight: '800',
    letterSpacing: 0.2,
  },
  compactShowFooterDate: {
    color: theme.muted,
    fontSize: 12,
    fontWeight: '500',
  },

  /** AllEventDetailScreen — dark redesign */
  detailScreen: {
    flex: 1,
    width: '100%',
    height: '100%',
    backgroundColor: theme.background,
  },
  detailListContent: {
    backgroundColor: theme.background,
    flexGrow: 1,
    paddingBottom: 36,
  },
  detailLoadingContainer: {
    ...StyleSheet.absoluteFillObject,
    backgroundColor: theme.background,
    alignItems: 'center',
    justifyContent: 'center',
  },
  detailOverlayHeaderWrap: {
    height: 0,
    overflow: 'hidden',
  },
  detailOverlayHeader: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
    paddingHorizontal: 16,
    paddingTop: 8,
    height: 56,
  },
  detailCircleBtn: {
    width: 36,
    height: 36,
    borderRadius: 18,
    backgroundColor: 'rgba(0, 0, 0, 0.45)',
    alignItems: 'center',
    justifyContent: 'center',
    borderWidth: StyleSheet.hairlineWidth,
    borderColor: 'rgba(255, 255, 255, 0.22)',
  },
  detailHeaderRight: {
    flexDirection: 'row',
    alignItems: 'center',
  },
  detailCircleBtnGap: {
    marginLeft: 8,
  },
  detailHeroWrap: {
    width: '100%',
    height: DETAIL_HERO_HEIGHT,
    marginBottom: 12,
    backgroundColor: theme.background,
    overflow: 'hidden',
  },
  detailHeroImage: {
    width: '100%',
    height: '100%',
  },
  detailHeroPlaceholder: {
    width: '100%',
    height: '100%',
    backgroundColor: theme.input,
    justifyContent: 'center',
    alignItems: 'center',
  },
  detailHeroScrim: {
    ...StyleSheet.absoluteFillObject,
    backgroundColor: 'rgba(8, 8, 15, 0.22)',
  },
  detailHeroFadeWrap: {
    position: 'absolute',
    left: 0,
    right: 0,
    bottom: 0,
    height: Math.round(DETAIL_HERO_HEIGHT * 0.82),
  },
  detailHeroFadeLayer: {
    position: 'absolute',
    left: 0,
    right: 0,
    backgroundColor: theme.background,
  },
  detailHeroTopBar: {
    position: 'absolute',
    top: 0,
    left: 0,
    right: 0,
    zIndex: 20,
  },
  detailHeroTopBarInner: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
    paddingHorizontal: 16,
    paddingTop: 8,
  },
  detailHeroActions: {
    position: 'absolute',
    top: 0,
    right: 16,
    zIndex: 12,
  },
  detailHeroMeta: {
    position: 'absolute',
    left: 16,
    right: 16,
    bottom: 14,
    zIndex: 5,
  },
  detailHotBadge: {
    alignSelf: 'flex-start',
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: theme.primary,
    borderRadius: 8,
    paddingHorizontal: 10,
    paddingVertical: 5,
    marginBottom: 8,
  },
  detailHotBadgeText: {
    color: '#FFFFFF',
    fontSize: 10,
    fontWeight: '800',
    letterSpacing: 1,
    marginLeft: 4,
  },
  detailHeroTitle: {
    color: '#FFFFFF',
    fontSize: 26,
    lineHeight: 30,
    fontWeight: '900',
    letterSpacing: 0.5,
    textTransform: 'uppercase',
    marginBottom: 8,
    textShadowColor: 'rgba(0, 0, 0, 0.88)',
    textShadowOffset: { width: 0, height: 1 },
    textShadowRadius: 8,
  },
  detailStateCaption: {
    color: 'rgba(255, 255, 255, 0.96)',
    fontSize: 12,
    fontWeight: '700',
    letterSpacing: 0.8,
    textTransform: 'uppercase',
    marginBottom: 6,
    textShadowColor: 'rgba(0, 0, 0, 0.88)',
    textShadowOffset: { width: 0, height: 1 },
    textShadowRadius: 6,
  },
  detailHeroLocation: {
    color: '#FFFFFF',
    fontSize: 13,
    lineHeight: 18,
    fontWeight: '600',
    marginTop: 8,
    textShadowColor: 'rgba(0, 0, 0, 0.88)',
    textShadowOffset: { width: 0, height: 1 },
    textShadowRadius: 6,
  },
  detailHeroMetaRow: {
    flexDirection: 'row',
    alignItems: 'center',
    flexWrap: 'wrap',
  },
  detailGenrePill: {
    borderRadius: 999,
    paddingHorizontal: 14,
    paddingVertical: 6,
    borderWidth: 1,
    borderColor: theme.primary,
    backgroundColor: theme.primarySoft,
    marginRight: 10,
    marginBottom: 4,
  },
  detailGenrePillText: {
    color: theme.primary,
    fontSize: 13,
    fontWeight: '700',
  },
  detailPriceText: {
    color: theme.primary,
    fontSize: 20,
    fontWeight: '800',
    marginBottom: 4,
  },
  detailBadgeRow: {
    flexDirection: 'row',
    alignItems: 'center',
    marginTop: 8,
  },
  detailStatusBadge: {
    width: 87,
    height: 28,
    borderRadius: 5,
    marginRight: 10,
  },
  detailSoldOutBadge: {
    width: 75,
    height: 28,
    borderRadius: 5,
  },
  detailSection: {
    paddingHorizontal: 16,
  },
  detailStatsBar: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    backgroundColor: theme.input,
    borderRadius: 12,
    borderWidth: 1,
    borderColor: 'rgba(196, 189, 214, 0.28)',
    paddingHorizontal: 18,
    paddingVertical: 12,
    minHeight: 48,
    marginBottom: 16,
  },
  detailStatItem: {
    flexDirection: 'row',
    alignItems: 'center',
  },
  detailStatCount: {
    color: theme.foreground,
    fontSize: 14,
    fontWeight: '700',
    marginLeft: 8,
  },
  detailStatSpacer: {
    flex: 1,
  },
  detailInfoCardsRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    marginBottom: 22,
  },
  detailInfoCard: {
    flex: 1,
    backgroundColor: theme.input,
    borderRadius: 16,
    borderWidth: 1,
    borderColor: 'rgba(196, 189, 214, 0.28)',
    paddingHorizontal: 12,
    paddingVertical: 14,
    marginHorizontal: 4,
  },
  detailInfoCardFirst: {
    marginLeft: 0,
  },
  detailInfoCardLast: {
    marginRight: 0,
  },
  detailInfoCardLabel: {
    color: theme.muted,
    fontSize: 12,
    fontWeight: '600',
    marginTop: 8,
    marginBottom: 4,
  },
  detailInfoCardValue: {
    color: theme.foreground,
    fontSize: 15,
    fontWeight: '800',
  },
  detailSectionLabel: {
    color: theme.muted,
    fontSize: 12,
    fontWeight: '800',
    letterSpacing: 1.2,
    marginBottom: 10,
  },
  detailLineupRow: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: theme.card,
    borderRadius: 14,
    borderWidth: StyleSheet.hairlineWidth,
    borderColor: theme.border,
    paddingHorizontal: 12,
    paddingVertical: 12,
    marginBottom: 8,
  },
  detailLineupIndex: {
    width: 28,
    height: 28,
    borderRadius: 14,
    alignItems: 'center',
    justifyContent: 'center',
    marginRight: 12,
    backgroundColor: theme.input,
  },
  detailLineupIndexActive: {
    backgroundColor: theme.primary,
  },
  detailLineupIndexText: {
    color: theme.muted,
    fontSize: 13,
    fontWeight: '800',
  },
  detailLineupIndexTextActive: {
    color: '#FFFFFF',
  },
  detailLineupName: {
    color: theme.foreground,
    fontSize: 15,
    fontWeight: '700',
    flex: 1,
  },
  detailAboutText: {
    color: theme.muted,
    fontSize: 14,
    lineHeight: 22,
    fontWeight: '400',
    marginBottom: 22,
  },
  detailVenueCard: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: theme.card,
    borderRadius: 16,
    borderWidth: StyleSheet.hairlineWidth,
    borderColor: theme.border,
    padding: 14,
    marginBottom: 10,
  },
  detailVenueIconWrap: {
    width: 44,
    height: 44,
    borderRadius: 12,
    backgroundColor: theme.input,
    alignItems: 'center',
    justifyContent: 'center',
    marginRight: 12,
  },
  detailVenueTextWrap: {
    flex: 1,
    paddingRight: 8,
  },
  detailVenueName: {
    color: theme.foreground,
    fontSize: 16,
    fontWeight: '800',
    marginBottom: 4,
  },
  detailVenueAddress: {
    color: theme.muted,
    fontSize: 13,
    lineHeight: 18,
    fontWeight: '400',
  },
  detailRulesHeader: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
    backgroundColor: theme.card,
    borderRadius: 16,
    borderWidth: StyleSheet.hairlineWidth,
    borderColor: theme.border,
    paddingHorizontal: 14,
    paddingVertical: 16,
  },
  detailRulesHeaderOpen: {
    borderBottomLeftRadius: 0,
    borderBottomRightRadius: 0,
    borderBottomWidth: 0,
  },
  detailRulesHeaderText: {
    color: theme.foreground,
    fontSize: 15,
    fontWeight: '700',
    flex: 1,
    paddingRight: 8,
  },
  detailRulesBody: {
    backgroundColor: theme.card,
    borderWidth: StyleSheet.hairlineWidth,
    borderColor: theme.border,
    borderTopWidth: 0,
    borderBottomLeftRadius: 16,
    borderBottomRightRadius: 16,
    paddingHorizontal: 14,
    paddingBottom: 14,
    marginBottom: 8,
  },
  detailRuleRow: {
    flexDirection: 'row',
    alignItems: 'flex-start',
    marginTop: 12,
  },
  detailRuleIcon: {
    marginRight: 10,
    marginTop: 1,
  },
  detailRuleTitle: {
    color: theme.foreground,
    fontSize: 14,
    fontWeight: '700',
    marginBottom: 2,
  },
  detailRuleText: {
    color: theme.muted,
    fontSize: 13,
    lineHeight: 20,
    fontWeight: '400',
    flex: 1,
  },
  detailMoreShowsRow: {
    paddingLeft: 16,
    marginBottom: 14,
  },
  detailMoreShowCard: {
    width: 148,
    height: 112,
    borderRadius: 16,
    overflow: 'hidden',
    marginRight: 10,
    backgroundColor: theme.card,
  },
  detailMoreShowImage: {
    width: '100%',
    height: '100%',
  },
  detailMoreShowFade: {
    position: 'absolute',
    left: 0,
    right: 0,
    bottom: 0,
    height: 64,
    backgroundColor: 'rgba(8, 8, 15, 0.7)',
  },
  detailMoreShowMeta: {
    position: 'absolute',
    left: 10,
    right: 10,
    bottom: 10,
  },
  detailMoreShowTitle: {
    color: '#FFFFFF',
    fontSize: 12,
    fontWeight: '800',
    textTransform: 'uppercase',
    marginBottom: 2,
  },
  detailMoreShowDate: {
    color: theme.accent,
    fontSize: 11,
    fontWeight: '700',
  },
  detailVenueProfileBtn: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    borderRadius: 16,
    borderWidth: 1,
    borderColor: theme.primary,
    paddingVertical: 14,
    marginBottom: 16,
  },
  detailVenueProfileBtnText: {
    color: theme.primary,
    fontSize: 15,
    fontWeight: '800',
    marginLeft: 8,
  },
  detailTicketsWrap: {
    marginTop: 8,
    marginBottom: 12,
  },
  detailPrimaryBtn: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: theme.primary,
    borderRadius: 16,
    paddingVertical: 16,
    width: '100%',
    shadowColor: theme.primary,
    shadowOpacity: 0.35,
    shadowRadius: 12,
    shadowOffset: { width: 0, height: 6 },
    elevation: 6,
  },
  detailPrimaryBtnText: {
    color: '#FFFFFF',
    fontSize: 16,
    fontWeight: '800',
    letterSpacing: 0.6,
    marginLeft: 8,
  },
  detailSecondaryBtn: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: 'transparent',
    borderRadius: 16,
    borderWidth: 1,
    borderColor: theme.divider,
    paddingVertical: 14,
    width: '100%',
    marginTop: 12,
  },
  detailSecondaryBtnText: {
    color: theme.foreground,
    fontSize: 15,
    fontWeight: '700',
  },
  detailWebsiteRow: {
    flexDirection: 'row',
    alignItems: 'flex-start',
    backgroundColor: theme.card,
    borderRadius: 16,
    borderWidth: StyleSheet.hairlineWidth,
    borderColor: theme.border,
    paddingHorizontal: 14,
    paddingVertical: 14,
    marginBottom: 10,
  },
  detailMetaLabel: {
    color: theme.muted,
    fontSize: 13,
    fontWeight: '700',
    marginRight: 8,
  },
  detailMetaValue: {
    color: theme.primary,
    fontSize: 14,
    fontWeight: '600',
    flex: 1,
  },
  detailShowTypeWrap: {
    marginBottom: 10,
  },
  detailFeatureRow: {
    flexDirection: 'row',
    width: '43%',
    marginRight: 10,
    marginBottom: 8,
    alignItems: 'flex-start',
  },
  detailFeatureText: {
    color: theme.muted,
    fontSize: 14,
    lineHeight: 22,
    fontWeight: '400',
    flex: 1,
  },
  detailErrorTitle: {
    fontSize: 16,
    fontWeight: '700',
    color: theme.foreground,
    textAlign: 'center',
    marginBottom: 8,
  },
  detailErrorBody: {
    fontSize: 14,
    color: theme.muted,
    textAlign: 'center',
    marginBottom: 24,
  },
  detailRetryBtn: {
    backgroundColor: theme.primary,
    paddingHorizontal: 24,
    paddingVertical: 12,
    borderRadius: 12,
    marginBottom: 12,
  },
  detailRetryText: {
    color: '#FFFFFF',
    fontWeight: '700',
  },
  detailGoBackText: {
    color: theme.primary,
    fontWeight: '700',
  },
  detailMenuContainer: {
    position: 'absolute',
    right: 16,
    top: 110,
    backgroundColor: theme.card,
    borderRadius: 12,
    padding: 8,
    minWidth: 220,
    borderWidth: StyleSheet.hairlineWidth,
    borderColor: theme.border,
    shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.4,
    shadowRadius: 8,
    elevation: 6,
    zIndex: 40,
  },
  detailMenuButtonText: {
    fontSize: 15,
    fontWeight: '500',
    color: theme.foreground,
    textAlignVertical: 'center',
  },
  detailHeroImageModalBackdrop: {
    flex: 1,
    backgroundColor: 'rgba(8, 8, 15, 0.62)',
    justifyContent: 'center',
    alignItems: 'center',
    paddingHorizontal: 24,
  },
  detailHeroImageModalImageWrap: {
    overflow: 'hidden',
  },
  detailHeroImageModalImage: {
    width: '100%',
    height: '100%',
  },
  detailHeroImageModalCloseBtn: {
    position: 'absolute',
    top: 8,
    right: 8,
    zIndex: 20,
  },
});

export const darkAllEventStyles = createAllEventStyles(redesignTheme);
export const lightAllEventStyles = createAllEventStyles(lightTheme);
export default darkAllEventStyles;


// Customizable Area End
